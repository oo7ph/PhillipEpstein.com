...
inception.prototype.handleEvent = function( event ) {
  if ( this[event.type] ) {
    this[event.type](event);
  }
};

inception.prototype.getScroll2DTransform = function( scroll ) {
  // 2D scale is exponential
  var scale = Math.pow( 3, scroll * (this.levels - 1) );
  return 'scale(' + scale + ')';
};

inception.prototype.getScroll3DTransform = function( scroll ) {
...
inception.prototype.scroll = function( event ) {
...
inception.prototype.transformScroll = function( scroll ) {
...
// handle click events
inception.prototype.click = function( event ) {
  ...
  // this will trigger window scroll event -> inception.prototype.scroll
...
inception.prototype.webkitTransitionEnd = function( event ) {
  this.transitionEnded( event );
};

inception.prototype.transitionend = function( event ) {
  this.transitionEnded( event );
};

inception.prototype.oTransitionEnd = function( event ) {
  this.transitionEnded( event );
};

// disables transition after nav click
inception.prototype.transitionEnded = function( event ) {
...
return window.inception = inception;
/*
$(function(){

  // BeerCamp '11 Global object
  // initialize inception
  var BCXI = new inception();
